...
abstract class SWebUser extends RWebUser 
{
    ...
    public function afterLogin($fromCookie)
    {
        ...
        if (!$this->isSuperuser && !$this->hasRole($this->currentRole)){
            //All accounts in multishop have the default role 'USER' assigned when signup 
            //However, each individual multishop-app might have additional role required to further restrict user access
            //e.g. multishop-merchant accounts need to have role 'MERCHANT' to manage shop 
            //So, newly registered account with role 'USER is allowed to login to multishop-merchant first but subject to access restriction by merchant app
