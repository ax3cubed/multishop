...
trait BraintreeApiTrait 
{
    ...
    protected $merchantCurrency;//If not set, will use the first merchant account id found in multishop.json
    /**
    ...
    protected function parseMerchantAccount()
    {
        ...
        if (isset($this->merchantCurrency) && isset($merchantAccounts[$this->merchantCurrency]))
            ...
            //Error when hit here! in the event $merchantAccounts is empty (happen only when multishop.json config is not correctly configured
            return null;
        }
    }
}
