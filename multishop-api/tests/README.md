...
3. Create `your_db_tests` database:

   Default test database name is `shopbay_codeception`
   Please change multishop-console database connection at `config.json`
   Run console command:

   ```
   $cd /path/to/multishop-console
   $php console database schema 
   ```

   Update it by applying migrations (you may skip this step if you do not have created any migrations yet):
   ...
   Or, run the following command to execute migration:
...
webserver. In the `web` directory execute the following:
   ...
   For acceptance test, by default, the api host is set as below (refer to acceptance.suite.yml)
   
   ```
   http://api.multishop.localhost/v1
   ```
